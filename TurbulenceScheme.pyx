...
cdef class Turbulence2ndOrder(TurbulenceBase):
    ...
    def plot_var2(self, str message, n1,n2, var1, var2, Grid Gr, ReferenceState Ref, TimeStepping TS, MeanVariables M1, SecondOrderMomenta M2):
        ...
        plt.legend(fontsize=8)
        plt.title('var1 '+v1+v2+ ', '+ np.str(np.round(np.amax(M2.values[n1,n2,gw:nzg-gw]),6)), fontsize=10)
        plt.subplot(1,5,5)
        ...
        plt.legend(loc=4,fontsize=8)
        plt.title('var2 ' + v1+v2+ ', '+ np.str(np.round(np.amax(M2.values[n1,n2,gw:nzg-gw]),6)), fontsize=10)
        # plt.subplot(1,4,4)
        # # plt.plot(var[w_varshift,:], Gr.z)
        # plt.title('aux')
        # plt.legend(fontsize=8)
