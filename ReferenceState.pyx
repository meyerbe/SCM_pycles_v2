...
cdef class ReferenceState:
    ...
    def initialize(self, Grid.Grid Gr):
        '''
        ...
        # (2b) Perform the integration of the hydrostatic equation
        #       (i) # Form a right hand side for integrating the hydrostatic equation to determine the reference pressure
        def rhs(p,z):
            # T, ql, qi = Thermodynamics.eos(np.exp(p), self.sg, self.qtg)
            # T, ql =  eos(t_to_entropy_c, eos_first_guess_entropy, np.exp(p),  self.qtg, self.sg)
            a = eos(np.exp(p), self.qtg, self.sg)
            qi = 0.0
            ql = a['ql']
            T = a['T']
        ...
        for k in xrange(Gr.nzg):
            ...
            alpha_half[k] = alpha_from_tp(p_half_[k], temperature_half[k], self.qtg, qv_half[k])
        ...
        for k in xrange(Gr.nzg):
            ...
            s = entropy_from_tp(p_half[k],temperature_half[k],self.qtg,ql_half[k],qi_half[k])
        ...
        # NS.write_reference_profile('alpha0', alpha_half[Gr.gw:-Gr.gw], Pa)
        # NS.add_reference_profile('p0', Gr, Pa)
        # NS.write_reference_profile('p0', p_half[Gr.gw:-Gr.gw], Pa)
        # NS.add_reference_profile('rho0', Gr, Pa)
        # NS.write_reference_profile('rho0', 1.0 / np.array(alpha_half[Gr.gw:-Gr.gw]), Pa)
        # NS.add_reference_profile('temperature0', Gr, Pa)
        # NS.write_reference_profile('temperature0', temperature_half[Gr.gw:-Gr.gw], Pa)
        # NS.add_reference_profile('ql0', Gr, Pa)
        # NS.write_reference_profile('ql0', ql_half[Gr.gw:-Gr.gw], Pa)
        # NS.add_reference_profile('qv0', Gr, Pa)
        # NS.write_reference_profile('qv0', qv_half[Gr.gw:-Gr.gw], Pa)
        # NS.add_reference_profile('qi0', Gr, Pa)
        # NS.write_reference_profile('qi0', qi_half[Gr.gw:-Gr.gw], Pa)
